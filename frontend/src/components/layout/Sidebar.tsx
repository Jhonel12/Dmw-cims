...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, collapsed, onToggleCollapse, onLogout }) => {
  ...
  return (
    <>
      ...
      `}>
        {/* Sidebar Header */}
        <div className={`border-b border-gray-200/60 ${collapsed ? 'p-4' : 'p-6'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <button
                onClick={onToggleCollapse}
                className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center hover:from-blue-600 hover:to-purple-700 transition-all duration-200 shadow-md"
        ...
        {/* Navigation */}
        <nav className={`space-y-2 ${collapsed ? 'p-4' : 'p-6'}`}>
          {menuItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
        ...
        {/* Sidebar Footer */}
        <div className={`absolute bottom-0 left-0 right-0 border-t border-gray-200/60 bg-gray-50/50 ${collapsed ? 'p-4' : 'p-6'}`}>
          {/* User Info */}
          ...
          <button
            ...
            className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-red-600 hover:bg-red-50 hover:text-red-700 transition-all duration-200 group ${
              collapsed ? 'justify-center px-2' : ''
            }`}
            title={collapsed ? 'Logout' : undefined}
          >
            <div className="flex items-center justify-center w-5 h-5 text-red-500 group-hover:text-red-600">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
